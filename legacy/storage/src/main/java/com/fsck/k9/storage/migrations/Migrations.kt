...
object Migrations {
    ...
    fun upgradeDatabase(db: SQLiteDatabase, migrationsHelper: MigrationsHelper) {
        ...
        if (oldVersion < 92) MigrationTo92(db, migrationsHelper).rebuildMessagesFulltextTable()
        if (oldVersion < 93) MigrationTo93(db, migrationsHelper).reindexMessagesFulltextWithDigitSplit()
    }
}
